/*
*Implementation of Vantage point tree using openmp
*Doinakis Michail
...
vptree * Vpt(double *X,int *IDS,int n, int d){
  ...
  if(n == 0)
    ...
    #pragma omp parallel for shared(X,distances,distance,n,d) schedule(dynamic, 4)
    for(int i = 0; i < n-1; i++){
      for(int j = 0; j < d; j++){
        distances[i] = distances[i] + pow((*(X+i*d+j)-vantageptree->vp[j]),2);
      }
      distances[i] = sqrt(distances[i]);
      distance[i] = sqrt(distances[i]);
    }
    ...
    for(int i=0; i<n-1;i++){
       ...
       // recursively calling our function in order to build all the sub-trees
      #pragma omp task shared(X)
      vantageptree->inner = Vpt(XInner,IDSin,innerelements,d);
      #pragma omp task shared(X)
